...
static NSString * const DB_NAME = @"TPFMDB.sqlite";
@interface TPFMDB ()
...
static TPFMDB *jqdb = nil;
+ (instancetype)shareDatabase
{
    return [TPFMDB shareDatabase:nil];
}

+ (instancetype)shareDatabase:(NSString *)dbName
{
    return [TPFMDB shareDatabase:dbName path:nil];
}
...
- (NSArray *)lookupAllData:(NSString *)tableName dicOrModel:(id)parameters {
    ...
    NSArray *arr = [self lookupTable:tableName dicOrModel:parameters whereFormat:@""];
    return arr;
}

- (NSArray *)lookupDataWithTableName:(NSString *)tableName dicOrModel:(id)parameters range:(NSInteger)range count:(NSInteger)count {
    
    //ORDER BY pkid ASC LIMIT 0,10
    NSString *format = [NSString stringWithFormat:@"ORDER BY pkid ASC LIMIT %ld,%ld",(long)range,(long)count];
    NSArray *arr = [self lookupTable:tableName dicOrModel:parameters whereFormat:format];
